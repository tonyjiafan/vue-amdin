      ...
      <template v-for="item in menuList">
        <template v-if="item.children && item.children.length === 1">
          <side-menu-item v-if="showChildren(item)"
                          class="whal_side_menu_item"
                          :key="`menu-${item.name}`"
                          :parent-item="item"></side-menu-item>
          <menu-item v-else
                      class="whal_menu_item"
                     :name="getNameOrHref(item, true)"
                     :key="`menu-${item.children[0].name}`">
            <common-icon :type="item.children[0].meta.icon || ''" /><span style="font-size: 14px;">{{ showTitle(item.children[0]) }}</span>
          </menu-item>
        </template>
        <template v-else>
          <side-menu-item v-if="showChildren(item)"
          ...
          <menu-item v-else
                     ...
                     :key="`menu-${item.name}`">
            <common-icon :type="item.meta.icon || ''" /><span style="font-size: 14px;">{{ showTitle(item) }}</span></menu-item>
        </template>
      </template>
    </Menu>
        ...
        <Tooltip transfer
          ...
          <a @click="handleSelect(getNameOrHref(item, true))"
            ...
            <common-icon :size="rootIconSize"
                         :color="textColor"
                         :type="item.meta.icon || (item.children && item.children[0].meta.icon)" /></a>
        </Tooltip>
      </template>
    </div>
  </div>
